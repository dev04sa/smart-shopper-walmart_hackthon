...
import { MdLogin, MdLogout } from "react-icons/md";
import { AiOutlineShoppingCart } from "react-icons/ai";
import { GiSunglasses } from "react-icons/gi"; // ✅ NEW ICON
import { Link, useNavigate } from "react-router-dom";
...
const Navbar = (props) => {
  ...
  return (
    <div className="">
      <div className="bg-[#0071dc] h-[100px]  px-3 py-2 lg:px-8 text-white flex justify-between items-center">
        {/* Mobile view starts*/}
        <div className="hamburger-icon mobileView" onClick={toggleSidebar}>
          <div className={`hamburger ${isSidebarOpen ? "open" : ""}`}>
        ...
        <div className={`sidebar ${isSidebarOpen ? "open" : ""}`}>
          <div className="sidebar-content">
            ...
            <div className="sidebar-item">
              <Link to="/VirtualTry" onClick={toggleSidebar}>Virtual Try-On</Link> {/* ✅ NEW ITEM */}
            </div>
            <div className="loginLinkSide sidebar-item">
              <MdLogout className="usern sidebar-icon" />
              <Link to="/login" onClick={toggleSidebar}>Login</Link>
        ...
        {/* Left */}
        <div className="flex  text-white  items-center gap-x-3 shrink-0">
          <TbBrandWalmart size={30} />

          <div className="px-1 rounded-full p-5   font-weight-bold text-4xl pt-24 fs-4 logo">Walmart </div>
          <div className="md:flex items-center gap-2 hidden hover:bg-[#06529a] p-3 rounded-full">
            <FaHome className="text-[17px]" />
            <Link to="/" className="text-[16px] text-white font-semibold">Home</Link>
          </div>
          <div className="md:flex hidden items-center gap-2 hover:bg-[#06529a] p-3 rounded-full ">
            <FiMail className="text-[20px]" />
            <p className="text-[16px] font-semibold">Contact Us</p>
        ...
        {/* Middle */}
        <div className="searchDiv   relative lg:flex items-center flex-1 max-w-[600px] mx-auto">
          <form className="relative flex-1" onSubmit={handleSearch}>
            <input type="search" placeholder="Search here" id="searchBar" className="rounded-full py-1.5 pl-4 pr-10 outline-0 w-full pr-10 text-black" />
            <div className="absolute bg-[#ffc220] p-1.5 rounded-full right-1.5 top-1/2 transform -translate-y-1/2">
              <GoSearch className="text-black" />
        ...
        <div className="flex  text-white  items-center gap-x-2">
          <div className="loginLink flex items-center gap-2 hover:bg-[#06529a] p-3 rounded-full whitespace-nowrap desktopView">
            <MdLogout className="usern text-[20px] -rotate-90" />
            <Link to="/login" className="text-[16px] text-white font-semibold">Login</Link>
          </div>
          <div className="dropdown">
            <a className="username dropdown-toggle" href="/" role="button" id="dropdownMenuLink" data-bs-toggle="dropdown" aria-expanded="false">
              <FaUser className="usern text-white text-[20px]" />
              <span className="name text-[16px] font-semibold"></span>
            </a>
            <ul className="dropdown-menu" aria-labelledby="dropdownMenuLink">
              <li><span className="dropdown-item text-white" style={{ cursor: 'pointer' }} onClick={logout}>Logout</span></li>
            </ul>
          </div>
          <div className="register flex items-center gap-2 hover:bg-[#06529a] p-3 rounded-full desktopView">
            <MdLogin className="regIcon text-[17px] rotate-90" />
            <Link to="/register" className="text-[16px] font-semibold text-white" onClick={registration}>Register</Link>
          </div>
          <div className="payment flex items-center gap-2 hover:bg-[#06529a] p-3 rounded-full desktopView">
            <FaHeart className="myIcon text-[17px]" />
            <Link to="/payment" className="text-[16px] text-white font-semibold">My Items</Link>
          </div>

          {/* ✅ New Desktop Nav Item */}
          <div className=" flex items-center gap-2 hover:bg-[#06529a] p-3 rounded-full desktopView">
            <GiSunglasses className="text-[20px]" />
            <Link to="/VirtualTry" className="text-[16px] text-white font-semibold">Virtual Try-On</Link>
          </div>

          <div className="hover:bg-[#06529a] p-3 rounded-full relative">
            <Link to="/checkout">
              <AiOutlineShoppingCart className="w-7 h-7" />
              <span className="absolute top-2  right-0 transform translate-x-[-20%]  translate-y-0.1 bg-red-500 text-white rounded-full px-1.5 py-0.5 text-xs" id="numItems"></span>
            </Link>
          </div>
        </div>
      </div>
